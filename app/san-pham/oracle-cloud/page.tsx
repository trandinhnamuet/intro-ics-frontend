'use client'

import { useLayoutEffect } from 'react'
import Image from 'next/image'
import Link from 'next/link'
import { useTranslation } from 'react-i18next'
import { Header } from '@/components/header'
import { Footer } from '@/components/footer'
import { Button } from '@/components/ui/button'
import { Card } from '@/components/ui/card'
import { Badge } from '@/components/ui/badge'
import { ScrollReveal } from '@/components/ui/scroll-reveal'
import { AnimatedHeading } from '@/components/ui/animated-heading'
import { Section } from '@/components/ui/section'
import { 
  Cloud, 
  Check, 
  ArrowRight, 
  Star, 
  Zap, 
  Server,
  Database,
  Shield,
  Globe,
  Award,
  ExternalLink,
  TrendingUp
} from 'lucide-react'

export default function OracleCloudPage() {
  const { t } = useTranslation()

  const features = [
    {
      icon: Server,
      title: t('products.oracleCloud.features.compute.title'),
      description: t('products.oracleCloud.features.compute.description')
    },
    {
      icon: Database,
      title: t('products.oracleCloud.features.database.title'),
      description: t('products.oracleCloud.features.database.description')
    },
    {
      icon: Shield,
      title: t('products.oracleCloud.features.security.title'),
      description: t('products.oracleCloud.features.security.description')
    },
    {
      icon: Globe,
      title: t('products.oracleCloud.features.network.title'),
      description: t('products.oracleCloud.features.network.description')
    }
  ]

  const benefits = [
    t('products.oracleCloud.benefits.item1'),
    t('products.oracleCloud.benefits.item2'),
    t('products.oracleCloud.benefits.item3'),
    t('products.oracleCloud.benefits.item4')
  ]

  // Server-side redirect is configured in next.config.mjs so this content is never
  // shown. This is a client-side fallback that replaces the current tab (no new tab).
  useLayoutEffect(() => {
    window.location.replace('https://oraclecloud.vn/')
  }, [])

  return (
    <>
      <Header />
      
      {/* Hero Section */}
      <div className="relative h-[600px] overflow-hidden mt-20">
        <div className="absolute inset-0 bg-gradient-to-br from-red-600 via-orange-600 to-red-700" />
        <div className="absolute inset-0 opacity-20">
          <div className="absolute top-0 left-1/4 w-96 h-96 bg-white rounded-full blur-3xl animate-pulse" />
          <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-white rounded-full blur-3xl animate-pulse delay-300" />
        </div>
        
        <div className="relative container-responsive h-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center h-full py-20">
            <ScrollReveal direction="left">
              <div>
                <Badge className="mb-6 px-4 py-2 bg-white/20 backdrop-blur-sm border-white/30 text-white">
                  <Cloud className="w-4 h-4 mr-2" />
                  {t('products.oracleCloud.badge')}
                </Badge>
                
                <h1 className="text-5xl lg:text-6xl font-bold text-white mb-6">
                  Oracle Cloud
                </h1>
                
                <p className="text-xl text-white/90 mb-8 leading-relaxed">
                  {t('products.oracleCloud.subtitle')}
                </p>
                
                <div className="flex flex-col sm:flex-row gap-4">
                  <Button 
                    size="lg" 
                    className="bg-white text-red-600 hover:bg-white/90 font-semibold text-lg"
                    asChild
                  >
                    <Link href="http://oraclecloud.vn/" target="_blank">
                      {t('products.oracleCloud.visitWebsite')}
                      <ExternalLink className="w-5 h-5 ml-2" />
                    </Link>
                  </Button>
                  <Button 
                    size="lg" 
                    variant="outline" 
                    className="bg-transparent border-2 border-white text-white hover:bg-white/10 font-semibold text-lg"
                  >
                    {t('common.contactUs')}
                  </Button>
                </div>
              </div>
            </ScrollReveal>
            
            <ScrollReveal direction="right">
              <div className="relative">
                <div className="absolute inset-0 bg-gradient-to-br from-white/20 to-white/5 rounded-3xl transform rotate-6" />
                <Card className="relative rounded-3xl overflow-hidden shadow-2xl border-4 border-white/20">
                  <Image
                    src="/doitac/Oracle.jpg"
                    alt="Oracle Cloud"
                    width={600}
                    height={400}
                    className="w-full h-auto"
                  />
                </Card>
              </div>
            </ScrollReveal>
          </div>
        </div>
      </div>

      {/* Features Section */}
      <Section spacing="sm" background="default">
        <div className="container-responsive">
          <ScrollReveal direction="up">
            <div className="text-center mb-16">
              <Badge className="mb-4 px-4 py-2">
                <Star className="w-4 h-4 mr-2" />
                {t('products.oracleCloud.features.title')}
              </Badge>
              <AnimatedHeading as="h2" gradient centered className="p-3 mb-6">
                {t('products.oracleCloud.features.heading')}
              </AnimatedHeading>
              <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
                {t('products.oracleCloud.features.description')}
              </p>
            </div>
          </ScrollReveal>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 auto-rows-max">
            {features.map((feature, idx) => (
              <ScrollReveal key={idx} direction="up" delay={idx * 100}>
                <Card className="p-8 hover:shadow-xl transition-all duration-500 hover:-translate-y-2 h-full flex flex-col">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center mb-6">
                    <feature.icon className="w-8 h-8 text-white" />
                  </div>
                  <h3 className="text-2xl font-bold mb-4">{feature.title}</h3>
                  <p className="text-muted-foreground leading-relaxed flex-grow">{feature.description}</p>
                </Card>
              </ScrollReveal>
            ))}
          </div>
        </div>
      </Section>

      {/* Benefits Section */}
      <Section spacing="sm" background="muted">
        <div className="container-responsive">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <ScrollReveal direction="left">
              <div>
                <Badge className="mb-4 px-4 py-2">
                  <Zap className="w-4 h-4 mr-2" />
                  {t('products.oracleCloud.benefits.title')}
                </Badge>
                <AnimatedHeading as="h2" gradient className="p-3 mb-6">
                  {t('products.oracleCloud.benefits.heading')}
                </AnimatedHeading>
                <p className="text-lg text-muted-foreground mb-8">
                  {t('products.oracleCloud.benefits.description')}
                </p>
                
                <div className="space-y-4">
                  {benefits.map((benefit, idx) => (
                    <div key={idx} className="flex items-start gap-3">
                      <div className="w-6 h-6 rounded-full bg-red-500 flex items-center justify-center shrink-0 mt-1">
                        <Check className="w-4 h-4 text-white" />
                      </div>
                      <p className="text-foreground">{benefit}</p>
                    </div>
                  ))}
                </div>
              </div>
            </ScrollReveal>
            
            <ScrollReveal direction="right">
              <Card className="p-8 bg-gradient-to-br from-red-50 to-orange-50 dark:from-red-950/20 dark:to-orange-950/20 border-2 border-red-200 dark:border-red-800">
                <div className="space-y-6">
                  <div className="flex items-center justify-between p-4 bg-white dark:bg-background rounded-lg">
                    <div>
                      <div className="text-sm text-muted-foreground">{t('products.oracleCloud.stats.regions')}</div>
                      <div className="text-3xl font-bold text-red-600">40+</div>
                    </div>
                    <Globe className="w-12 h-12 text-red-500" />
                  </div>
                  <div className="flex items-center justify-between p-4 bg-white dark:bg-background rounded-lg">
                    <div>
                      <div className="text-sm text-muted-foreground">{t('products.oracleCloud.stats.uptime')}</div>
                      <div className="text-3xl font-bold text-green-600">99.99%</div>
                    </div>
                    <TrendingUp className="w-12 h-12 text-green-500" />
                  </div>
                  <div className="flex items-center justify-between p-4 bg-white dark:bg-background rounded-lg">
                    <div>
                      <div className="text-sm text-muted-foreground">{t('products.oracleCloud.stats.customers')}</div>
                      <div className="text-3xl font-bold text-red-600">430K+</div>
                    </div>
                    <Award className="w-12 h-12 text-red-500" />
                  </div>
                </div>
              </Card>
            </ScrollReveal>
          </div>
        </div>
      </Section>

      {/* CTA Section */}
      <Section spacing="sm" background="gradient">
        <div className="container-responsive">
          <ScrollReveal direction="up">
            <Card className="relative overflow-hidden bg-gradient-to-br from-red-600 via-orange-600 to-red-700 p-12 lg:p-20 text-center border-none">
              <div className="absolute inset-0 opacity-10">
                <div className="absolute top-0 left-1/4 w-96 h-96 bg-white rounded-full blur-3xl animate-pulse" />
                <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-white rounded-full blur-3xl animate-pulse delay-300" />
              </div>
              <div className="relative z-10">
                <div className="w-20 h-20 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center mx-auto mb-6">
                  <Cloud className="w-10 h-10 text-white" />
                </div>
                <h2 className="text-4xl lg:text-5xl font-bold text-white mb-6">
                  {t('products.oracleCloud.cta.heading')}
                </h2>
                <p className="text-xl text-white/90 mb-8 max-w-2xl mx-auto">
                  {t('products.oracleCloud.cta.description')}
                </p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Button 
                    size="lg" 
                    className="text-lg px-8 bg-white text-red-600 hover:bg-white/90 font-semibold"
                    asChild
                  >
                    <Link href="http://oraclecloud.vn/" target="_blank">
                      {t('products.oracleCloud.visitWebsite')}
                      <ExternalLink className="w-5 h-5 ml-2" />
                    </Link>
                  </Button>
                  <Button 
                    size="lg" 
                    className="text-lg px-8 bg-white/20 text-white border-2 border-white hover:bg-white/30 font-semibold backdrop-blur-sm"
                  >
                    {t('common.contactUs')}
                    <ArrowRight className="w-5 h-5 ml-2" />
                  </Button>
                </div>
              </div>
            </Card>
          </ScrollReveal>
        </div>
      </Section>
      
      <Footer />
    </>
  )
}
